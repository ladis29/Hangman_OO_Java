...
public class Game {
	...
	public void game() {
		...
		while (!leveler.getFinished()) {
			...
			// Após preparar as variáveis para o início da rodada vamos começar a rodada em
			// si
			while (!wordWasDiscovered && lostGuess != leveler.getLevelMaxGuesses()) {
				/*
				 * A variável wordWasDiscovered está como condição para esse laço pq se
				 * a palavra do level 1 for descoberta ele sai do loop para entrar no level 2 ou
				...
				// As próximas 5 linhas montam a tela de jogo
				screen.clearScreen();
				...
				System.out.print(messages.getCommandToType());
				guess = sc.next().charAt(0);
				if(typed.verifyTyped(guess)) {
					played++;
					System.out.println(messages.getAlreadyTyped());
					sc.nextLine();
					...
					if (guess == '_') {
						...
						for (int i = 0; i < wordToGuess.length; i++) {
							...
							if (wordToGuess[i] == guess) {
								...
								firstloop+=1;//Criei a variável firstloop porque quando eu acertava uma letra que se repetia
								//ele decrementava a variável lostGuess a cada repetição
								if (firstloop == 1)lostGuess--;
			...
			if (!wordWasDiscovered) {
				screen.clearScreen();
				screen.gibbet(lostGuess, level);
				System.out.print(messages.getFailed());
				screen.printArray(wordToGuess);
				System.out.println();
				leveler.setFinished(true);
